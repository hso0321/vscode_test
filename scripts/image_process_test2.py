...
def prev_window_refer(b_img, left_line, right_line):
    ...
    temp, count_g, count_h = 0, 0, 0
    for i, j in enumerate(left_line.allx):      # i y값 j 라인 x값
        for m in range(window_margin):          # 윈도우 폭의 반
            j1, j2 = int(j) + m, int(j) - m     # j1 윈도우 좌측 j2 윈도우 우측

            if b_img[i, j1] >= 70 and b_img[i, j1] <= 130:
                temp += 0.2 * j1                            # 0.2j1 만큼 temp 증가
                count_g += 1
                output[i, j1] = (255, 0, 0)
            if b_img[i, j2] >= 70 and b_img[i, j2] <= 130:      # 회색이면 count_g ++, temp += 0.2*j
                temp += 0.2 * j2
                count_g += 1
                output[i, j2] = (255, 0, 0)
            if b_img[i, j1] > 220:                              # 흰색이면 count_h ++, temp += 0.8j
                temp += 0.8 * j1
                count_h += 1
                output[i, j1] = (0, 0, 255)
            if b_img[i, j2] > 220:
...
""" ros와 line detect 구문 분리용 주석"""
    ...
    try:
        ...
        undist_img = undistort(cv2_img, mtx, dist)
        # resize video
